...
def build_career_rows(cur, current_day: int, starting_year: int = 2025):
    ...
    for row in cur.execute(
        ...
        if rt == 0 and pos == 1:
            ...
            t["national_cup"] += 1
        # rt1 = colocação na copa (não título); não somar para não misturar com supercopa/copa
    ...
    def one_player(row, status: str):
        ...
        # Mapa h_rewards:
        #   0 artilheiro (global → Chuteira)
        #   1 maior assistência
        #   2 global → Bola de Ouro; senão → NÃO usado como time do ano
        #   3 Golden Boy (somente global)
        #   4 melhor goleiro (global → Luva)
        #   5 seleção do torneio (copas / continental / mundial)
        #   6 time do ano (Best XI)
        # Jogador do ano (competições) = o que antes ia para "Time do ano — ligas" (tipo 5+6 em CH)
        # Time do ano — ligas = somente tipo 6 em CH
        for typ, comp, cnt in rw:
            b = reward_bucket(comp)
            if typ == 6:  # time do ano (Best XI)
                totm["toy_total"] += cnt
                if b == "league":
                    totm["toy_league"] += cnt
                elif b == "national_cup":
                    ...
                    totm["sel_wc"] += cnt
            elif typ == 5:  # seleção do torneio
                totm["toy_total"] += cnt
                if b == "league":
                    # Liga: tipo 5 entra em Jogador do ano (competições), não em Time do ano
                    totm["mvp"] += cnt
                elif b == "national_cup":
                    totm["toy_cup"] += cnt
                elif b == "continental":
                ...
                if b == "global":
                    totm["golden_boy"] += cnt  # só global
            elif typ == 2:
                if b == "global":
                    totm["ballon"] += cnt
                # tipo 2 doméstico não entra em Jogador do ano nem Time do ano

        # Jogador do ano (competições) = o que "Time do ano — ligas" fazia antes: tipo 5 CH + tipo 6 CH
        # Time do ano — ligas = somente tipo 6 CH
        jogador_ano = totm["mvp"] + totm["toy_league"]  # tipo5 CH + tipo6 CH
        ...
        return [
            ...
            *role_cols,
            ...
            jogador_ano,  # Jogador do ano (competições) = antigo Time do ano ligas (5+6 CH) + tipo2
            int(total_clean),
            r1(total_clean / seasons) if seasons else None,
            mk,
            totm["toy_league"],  # Time do ano — ligas = só tipo 6 CH
            totm["toy_cup"],
            totm["sel_cont"],
            totm["sel_wc"],
            totm["art_league"],
...
def create_workbook(data, output_path, language="pt"):
    ...
    write_sheet(wb, "Dicionário", "Significado das Colunas", ["Campo", "Leitura"], [
        ...
        ["Carreira — prêmios", "0 artilheiro (global=Chuteira), 1 assistência, 2 global=Bola de Ouro, 3 Golden Boy só global, 4 melhor GK (global=Luva), 5 seleção do torneio, 6 time do ano"],
        ["Jogador do ano (competições)", "Tipo 5+6 em ligas (CH) — o que antes aparecia em Time do ano — ligas"],
        ["Time do ano — ligas", "Somente tipo 6 em CH (Best XI da liga)"],
        ["Títulos supercopa", "rt 3/4 = supercopa nacional; rt 6 = supercopa continental; rt 2 = continental; rt 7 = copa nacional"],
