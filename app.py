...
def run_streamlit_app() -> None:
    ...
    if generate and uploaded is not None:
        ...
        # Redireciona emit() para a barra do Streamlit sem mudar o restante do código
        global emit
        original_emit = emit
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="fm26-st-") as temporary_dir:
                save_path = Path(temporary_dir) / uploaded.name
                save_path.write_bytes(uploaded.getvalue())
                out_name = f"Moneyball_{Path(uploaded.name).stem}.xlsx"
                ...
                st.success(
                    f"**{data.get('club', 'Save')}** — data interna **{data.get('current_day', '?')}** · "
                    f"{len(data.get('players', []))} jogadores · {len(data.get('gks', []))} goleiros · "
                    f"{len(data.get('teams', []))} times"
                )
                st.download_button(
