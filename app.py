...
CAREER_HEADERS = [
    ...
    "Hat-tricks na carreira", "Hat-tricks/temporada",
    ...
    "Jogador do ano (competições)",
    "Clean sheets na carreira", "Clean sheets/temporada",
    "Valor de mercado atual €", "Maior valor de mercado carreira €",
    ...
    "Vezes no time do ano", "Chuteira de ouro", "Luva de ouro", "Golden Boy", "Bola de ouro",
]
...
def build_team_rows(cur, date_info, players, played_matches, team_cards):
    ...
    for team_id, team in teams.items():
        # teams2 NÃO tem league_id — liga vem de champ_id → champs2.league_id → leagues2
    ...
    for tid, t in teams.items():
        # Inclui time com jogos OU com elenco (início de temporada / só fixtures futuras)
        if not t["all"]["games"] and not t["players"]:
            continue
        all_d, bef, aft = t["all"], t["before"], t["after"]
        # overall do time: team_in_time_value (histórico) + value atual
        hist = parse_values(t.get("team_in_time_value"))
        ov_final = round(sf(t.get("value")) or (hist[-1] if hist else 0), 2)
        ov_init = round(hist[0], 2) if hist else ov_final
        ov_aug = round(hist[len(hist) // 2], 2) if len(hist) > 2 else ov_final
        roster = t["players"]
        avg_sal = (
        ...
        rows.append({
            "name": t["name"],
            "league": t["league"],
            "continent": t.get("champ_name") or t.get("continent_champ_name") or "Nenhuma",
            "players_count": len(roster),
            "avg_age": sum(sf(p.get("age")) for p in roster) / len(roster) if roster else 0,
            "all": all_d, "before": bef, "after": aft,
            "yellow_cards": t["yellow_cards"], "red_cards": t["red_cards"],
            "ov_init": ov_init, "ov_aug": ov_aug, "ov_final": ov_final,
...
def parse_reward_items(raw):
    ...
    Tipos (validado no save):
      0 = artilheiro (global → Chuteira de ouro)
      1 = maior assistência
      2 = global → Bola de Ouro; senão MVP/jogador da competição
      3 = Golden Boy
      4 = melhor goleiro (global → Luva de ouro)
      5 = seleção do torneio (mesmo conceito de time do ano)
      6 = time do ano (Best XI)
    """
    items = []
...
def build_career_rows(cur, current_day: int, starting_year: int = 2025):
    ...
    def one_player(row, status: str):
        ...
        totm = {
            "toy_league": 0, "toy_cup": 0, "sel_cont": 0, "sel_wc": 0,
            ...
            "toy_total": 0, "golden_boot": 0, "golden_glove": 0, "golden_boy": 0, "ballon": 0,
            "mvp": 0,
        }
        # Mapa h_rewards (validado no save):
        #   0 = artilheiro (global → Chuteira de ouro)
        #   1 = maior assistência
        #   2 = Bola de Ouro / melhor do mundo quando global; senão MVP/jogador da competição
        #   3 = Golden Boy
        #   4 = melhor goleiro (global → Luva de ouro)
        #   5 = seleção do torneio (mesmo conceito de "time do ano", nome diferente)
        #   6 = time do ano (Best XI da competição)
        for typ, comp, cnt in rw:
            b = reward_bucket(comp)
            if typ == 6:  # time do ano (Best XI da competição)
                totm["toy_total"] += cnt
                if b == "league":
                    totm["toy_league"] += cnt
                elif b == "national_cup":
                    totm["toy_cup"] += cnt
                elif b in ("continental", "wc", "global"):
                    # Best XI continental/mundial também conta no total
                    if b == "continental":
                        totm["sel_cont"] += cnt
                    elif b == "wc":
                        totm["sel_wc"] += cnt
            elif typ == 5:  # seleção do torneio (nome alternativo do time do ano em copas)
                # Yamal: 4×copa ESP + 1×UCL + 1×WC = 6 (tipo 5)
                totm["toy_total"] += cnt
                if b == "league":
                    totm["toy_league"] += cnt
                elif b == "national_cup":
                    ...
                    totm["sel_wc"] += cnt
                # tipo 5 global = seleção mundial do ano — entra só no total
            elif typ == 0:  # artilheiro
                if b == "global":
                ...
                if b == "global":
                    ...
                    totm["gk_wc"] += cnt
            elif typ == 3:  # Golden Boy (global ou local)
                totm["golden_boy"] += cnt
            elif typ == 2:
                if b == "global":
                    totm["ballon"] += cnt  # Bola de Ouro
                else:
                    # Jogador do ano da competição (NÃO é MVP de partida)
                    totm["mvp"] += cnt  # reutilizado na coluna "MVPs" só se não houver partidas; ver nota no dicionário
                    # Na prática a coluna da planilha deve ser lida como "Jogador do ano (comp.)" quando sem partidas no DB
        ...
        return [
            ...
            *role_cols,
            ...
            int(nat_goals),
            None,  # hat-tricks — não gravados no save
            None,  # hat-tricks/temporada
            int(total_assists),
            ...
            mk,
            None,  # maior valor de mercado — histórico de mercado não existe no save
            totm["toy_league"],
            totm["toy_cup"],
            totm["sel_cont"],
            totm["sel_wc"],
...
def create_workbook(data, output_path, language="pt"):
    ...
    write_sheet(wb, "Dicionário", "Significado das Colunas", ["Campo", "Leitura"], [
        ...
        ["Carreira — prêmios", "0 artilheiro (global=Chuteira), 1 assistência, 2 global=Bola de Ouro / senão Jogador do ano da competição, 3 Golden Boy, 4 melhor GK (global=Luva), 5 seleção do torneio, 6 time do ano"],
        ["Jogador do ano (competições)", "h_rewards tipo 2 fora de global — NÃO é MVP de partida"],
        ...
        ["Hat-trick / pico de mercado", "Não existem no save — colunas ficam vazias"],
    ], {})
