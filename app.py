...
def build_career_rows(cur, current_day: int, starting_year: int = 2025):
    ...
    def one_player(row, status: str):
        ...
        # Seleção: segmentos com flag nacional OU team_id == -1 com jogos
        ...
        for i in range(n):
            ...
            national = (is_nat[i] >= 1) or (int(team_ids[i]) == -1 and p > 0 and is_nat[i] == 0 and len([x for x in is_nat if x >= 1]) == 0 and int(team_ids[i]) == -1)
            # Prefer explicit h_is_national when present
            if any(x >= 1 for x in is_nat):
                national = is_nat[i] >= 1
            else:
                national = int(team_ids[i]) == -1
            if national:
                nat_games += p
                nat_goals += g
                nat_assists += a
        ...
        for typ, comp, cnt in rw:
            ...
            if typ == 6:  # time do ano (Best XI)
                ...
                if b == "league":
                    ...
                    totm["toy_total"] += 0  # já contou acima; world XI entra no total
            elif typ == 5:  # seleção do torneio (= time do ano com outro nome)
                totm["toy_total"] += cnt
                if b == "league":
                    totm["toy_league"] += cnt
                elif b == "national_cup":
                    ...
                    totm["sel_wc"] += cnt
                # global type 5 = seleção mundial do ano (FIFA XI etc.), não Bola de Ouro
...
def create_workbook(data, output_path, language="pt"):
    ...
    write_sheet(wb, "Dicionário", "Significado das Colunas", ["Campo", "Leitura"], [
        ...
        ["Carreira — seleção", "Segmentos com h_is_national=1; se ausente, team_id=-1"],
        ["Carreira — prêmios", "h_rewards: 0 artilheiro (global=Chuteira), 1 assistência, 2 global=Bola de Ouro, 3 Golden Boy, 4 melhor GK (global=Luva), 5 seleção do torneio, 6 time do ano"],
